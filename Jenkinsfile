...
def pullEngineImage() {
  sh 'docker pull axonivy/axonivy-engine:nightly-8.0'
}
...
def examples() {
  return [
    ...
    'ivy-logging': { assertIvyConsoleLog("ivy-logging", "Loaded configurations of '/usr/lib/axonivy-engine/configuration/ivy.yaml'") },
...
def assertOpenLdap() {
  ...
  def response = sh (script: "curl 'http://localhost:8080/ivy/pro/ldap/quick-start-tutorial/148655DDB7BB6588/start.ivp' --user rwei:rwei -L -i -b cookie.txt -s -o /dev/null -D/dev/stdout", returnStdout: true)
